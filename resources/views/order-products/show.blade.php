                            ...
                            <tbody>
                                @foreach ($order->orderItems as $orderItem)
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <b>{{ number_format($orderItem->discount, null, ',', '.') }}
                                                {{ $orderItem->discount > 100 ? '' : '%' }}
                                                {{ $orderItem->discount > 100 ? '' : '(' . number_format($orderItem->bruto * $orderItem->quantity * ((int) $orderItem->discount / 100), null, ',', '.') . ')' }}</b>
                                        </td>
                                        <td class="text-center">
    ...
    <div class="row">
        <div class="col-md-12 total-items">
            <div class="box box-primary">
                ...
                <div class="box-body">
                    <form id="fileupload" action="{{ route('order-products.store') }}" method="post"
                        enctype="multipart/form-data" autocomplete="off">
                        <input type="hidden" name="customer_id" id="customer_id" value="{{ $order->customer->id }}">
                        {{ csrf_field() }}
                        <p class="margin-b-2" style="display: none;"><b>Bruto: </b><span
                                id="bruto">{{ number_format($order->bruto, null, ',', '.') }}</span></p>
                        <p class="margin-b-2"><b>Sub total: </b><span
                                id="sub_total">{{ number_format($order->bruto, null, ',', '.') }}</span></p>
                        <p class="margin-b-2"><b>Discount: </b><input type="text" id="discount" readonly
                                name="discount"
                                value="{{ $order->discount == 0 ? '' : number_format($order->discount, null, ',', '.') }}"
                                class="form-control" style="display: inline"></p>
                        <p class="margin-b-2" style="display: none;"><b>Netto: </b><span
                                id="netto">{{ number_format($order->netto, null, ',', '.') }}</span></p>
                        <p class="margin-b-2"><i>INCLUDED PPN: </i><span id="tax">{{ $config->vat }}%</span></p>
                        <p class="margin-b-2"><b>Total: </b><span
                                id="total">{{ number_format($order->netto, null, ',', '.') }}</span></p>
        ...
        <div class="row">
            <div class="col-md-12 total-items">
                ...
                <button type="button" class="btn bg-purple" id="btn-pickup" data-toggle="modal"
                    data-target="#modal-pickup-item" style="margin-bottom: 15px;">
                ...
                <div id="modal-pickup-item" class="modal fade" role="dialog" data-keyboard="false"
                    ...
                    <div class="modal-dialog modal-sm">
                        <!-- Modal content-->
                        <form id="pickup-item-form" action="{{ route('order-products.update', $order->id) }}"
                            method="post" onsubmit="pickUpItemForm(event)" autocomplete="off">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <button type="button" class="close" data-dismiss="modal">&times;</button>
                ...
                <!-- Save Button -->
                <button type="button" class="btn bg-olive" id="btn-simpan"
                    style="margin-bottom: 15px; margin-right: 10px;" onclick="pickUpItemForm(event)">
                    <i class="glyphicon glyphicon-file"></i>
                    <span>Simpan</span>
                </button>

                <!-- Print Buttons Grouped with Spacing -->
                <div class="btn-group" role="group" aria-label="Print Options" style="margin-bottom: 15px;">
                    <a href="{{ route('order-products.print', $order->id) }}?type=customer" target="_blank"
                        class="btn bg-navy" style="margin-right: 5px;">
                        <i class="fa fa-fw fa-print"></i>
                        <span>Customer</span>
                    </a>
                    <a href="{{ route('order-products.print', $order->id) }}?type=cashier" target="_blank"
                        class="btn bg-navy" style="margin-right: 5px;">
                        <i class="fa fa-fw fa-print"></i>
                        <span>Cashier</span>
                    </a>
                    {{-- <a href="{{ route('order-products.print', $order->id) }}?type=reparation" target="_blank" class="btn bg-navy"> --}}
                    {{--     <i class="fa fa-fw fa-print"></i> --}}
                ...
                <div class="text-right" style="margin-top: 15px;">
                    <a href="{{ route('order-products.index') }}" class="btn btn-default">
    ...
    @else
        <div class="row">
            <div class="col-md-12 total-items">
                ...
                <div class="vl"
                    style="display:inline-block; border-product-left: 1px solid black; height: 30px; margin: auto 15px;">
                </div>
                &nbsp;
                <a href="{{ route('order-products.print', $order->id) }}?type=customer" target="_blank"
                    class="btn bg-navy" style="margin-left:15px;margin-right: 15px;margin-top: -20px;">
                    <i class="fa fa-fw fa-print"></i>
                    <span>Customer</span>
                </a>
                <a href="{{ route('order-products.print', $order->id) }}?type=cashier" target="_blank"
                ...
                {{--     <span>Reparation</span> --}}
                {{-- </a> --}}
                <a href="{{ route('order-products.index') }}" class="btn btn-default pull-right"><i
...
@section('layout_js')
    ...
    <script>
        ...
        if (status.toLowerCase() === 'diambil') {
            $('#btn-add-customer, #btn-add-item, #customer, #note, #site_id, #btn-pickup, #btn-simpan, .btn-edit, .btn-remove')
                .attr("disabled", true);
        }
        ...
        function showEditItemForm(index) {
            ...
            if (dataItem.discount_item > 100) {
                ...
                $('#total_after_discount_edit').text((dataItem.bruto * (dataItem.discount_item / 100)).toLocaleString(
        ...
        function renderItems() {
            ...
            items.forEach(function(item, index) {
                ...
                let info_discount = parseInt(item.discount_item, 10) > 100 ? item.discount_item : item.jumlah * (
        ...
        function sumTotalItem() {
            ...
            for (let i = 0; i < items.length; i++) {
                ...
                discount += parseInt(items[i].discount_item, 10) > 100 ? Number(items[i].discount_item) : (parseInt(items[i]
                    .bruto, 10)) * (parseInt(items[i].discount_item, 10) / 100);
        ...
        $(function() {
            ...
            $('#kuantitas_item').on('input', function(e) {
                ...
                if (discount_item === '') {
                    ...
                    if (Number(discount_item) > 100) {
                        total_item.val(total_harga === 0 ? '' : (total_harga - Number(discount_item))
                        ...
                        total_item.val(total_harga === 0 ? '' : (total_harga - total_after_discount)
                            .toLocaleString('id-ID'));
                    }
                }
            });
            ...
            $('#discount_item').on('input', function(e) {
                ...
                if (discount_item === '') {
                    ...
                    if (Number(digitsOnly) > 100) {
                        total_item.val(total_harga === 0 ? '' : (total_harga - Number(digitsOnly))
                            .toLocaleString('id-ID'));
                        $('#total_after_discount').text('').parent().css('display', 'none');
                    } else {
            ...
            $('#kuantitas_item_edit').on('input', function(e) {
                ...
                if (discount_item_edit === '') {
                    ...
                    if (Number(discount_item_edit) > 100) {
                        total_item_edit.val(total_harga === 0 ? '' : (total_harga - Number(
                            discount_item_edit)).toLocaleString('id-ID'));
                        $('#total_after_discount_edit').text('').parent().css('display', 'none');
                    } else {
                        ...
                        $('#total_after_discount_edit').text(total_after_discount_edit.toLocaleString(
                            'id-ID')).parent().css('display', 'inline');
                        total_item_edit.val(total_harga === 0 ? '' : (total_harga -
            ...
            $('#discount_item_edit').on('input', function(e) {
                ...
                if (discount_item_edit === '') {
                    ...
                    if (Number(digitsOnly) > 100) {
                        total_item_edit.val(total_harga === 0 ? '' : (total_harga - Number(digitsOnly))
                            .toLocaleString('id-ID'));
                        $('#total_after_discount_edit').text('').parent().css('display', 'none');
                    } else {
                        const total_after_discount_edit = total_harga * (Number(digitsOnly) / 100);
                        if (Number(digitsOnly) === 100) {
                            total_item_edit.val(total_harga === 0 ? '' : total_harga.toLocaleString(
                                'id-ID'));
                            $('#total_after_discount_edit').text('').parent().css('display', 'none');
                        } else {
                            $('#total_after_discount_edit').text(total_after_discount_edit.toLocaleString(
            ...
            $('#discount').on('input', function(e) {
                ...
                const vatRate = parseInt("{{ $config->vat }}", 10);
