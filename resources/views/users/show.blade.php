                    ...
                    <div class="col-md-12">
                        ...
                        <div class="col-lg-9">
                            <div class="attachment">
                                ...
                                @endforeach
                                <br><br>
                                <p class="help-block"><i class="fa fa-clock-o"></i> Created on:
                                    {{ $user->created_at->format('d/m/Y H:i') }}</p>
                                <p class="help-block"><i class="fa fa-refresh"></i> Last update:
                                    {{ $user->updated_at->format('d/m/Y H:i') }}</p>
                                <br>
                                <div class="pull-right">
                                    <a href="{{ route('user.edit.password', $user->id) }}"
                                        title="Change Password {{ $user->name }}"><button type="button"
                                            class="btn btn-primary btn-sm btn-flat"><i class="fa fa-key"></i> Change
                                            Password</button></a>
                                    <a href="{{ route('user.edit', $user->id) }}" title="Edit {{ $user->name }}"><button
                                            type="button" class="btn btn-warning btn-sm btn-flat"><i
