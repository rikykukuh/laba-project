        ...
        <div class="col-md-12 total-items">
            <div class="box box-info">
                ...
                <div class="box-body">
                    ...
                    <div class="form-group">
                        <label for="amount_payment">Nominal Pembayran: <small class="text-danger">*</small></label>
                        <input type="text" class="form-control" name="amount_payment" id="amount_payment" required>
                    </div>
                    <div class="custom-control custom-switch">
                      <input type="checkbox" class="custom-control-input" id="toggle_split_payment">
                      <label class="custom-control-label" for="toggle_split_payment">Split Payment</label>
                    </div>
                </div>
            </div>
        </div>

        <div class="col-md-12 total-items hidden" id="split_group">
            <div class="box box-info">
                <div class="box-header with-border-product">
                    <h3 class="box-title">Split Pembayaran</h3>
                    <div class="box-tools pull-right">
                        <button type="button" class="btn btn-box-tool" data-widget="collapse" data-toggle="tooltip"
                            title="" data-original-title="Collapse Form Pembayaran">
                            <i class="fa fa-minus"></i></button>
                ...
                <div class="box-body">
                    <div class="form-group">
                        <label for="split_payment_method">Metode Pembayaran: <small class="text-danger">*</small></label>
                        <select class="form-control" id="split_payment_method" name="split_payment_method" required>
                            @foreach ($payment_methods as $payment_method)
                                <option value="{{ $payment_method->id }}">{{ $payment_method->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        <label for="split_payment_merchant">Penyedia Pembayaran: <small class="text-danger">*</small></label>
                        <select class="form-control" id="split_payment_merchant" name="split_payment_merchant" required>
                            <option value="1">-</option>
                        </select>
                    </div>
                    <div class="form-group">
                        <label for="split_amount_payment">Nominal Pembayran: <small class="text-danger">*</small></label>
                        <input type="text" class="form-control" name="split_amount_payment" id="split_amount_payment" required>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="box-body">
                    <form action="{{ route('orders.store') }}" method="post" enctype="multipart/form-data"
                        ...
                        <p class="margin-b-2"><b>Pembayaran diterima: </b><input type="text" id="dp"
                                name="dp" value="" readonly class="form-control" style="display: inline"></p>
...
@section('layout_js')
    ...
    <script>
        ...
        $('#toggle_split_payment').on('change', function () {
            if (this.checked) {
                $('#split_group').removeClass('hidden');
            } else {
                $('#split_group').addClass('hidden');
            }
        });
        ...
        function updateDp() {
            const amount = cleanNumber($('#amount_payment').val());
            const split  = cleanNumber($('#split_amount_payment').val());

            $('#dp').val(amount + split);
            $('#dp').trigger('input');
        }

        $('#amount_payment, #split_amount_payment').on('keyup change', updateDp);
        ...
        $('#split_payment_method').on('change', function() {
            getSplitPaymentMerchant();
        });

        function getSplitPaymentMerchant() {
            $.ajax({
                url: '{{ route('order-products.merchant_by_payment') }}',
                type: 'GET',
                dataType: 'json',
                ...
                success: function(response) {
                    $('#split_payment_merchant').empty();
                    $.each(response, function(index, paymentMerchant) {
                        $('#split_payment_merchant').append('<option value="' + paymentMerchant.id + '">' +
                            paymentMerchant.name + '</option>');
                    });
                },
                error: function(xhr, status, error) {
        ...
        function compressBase64(base64, maxWidth = 800, quality = 0.7) {
        ...
        async function saveItem(event) {
            ...
            const discount_item = $('#discount_item').val() !== '' 
                ? parseInt($('#discount_item').val().replace(/\./g, ''), 10) 
                : 0;
        
            let gambar = [];

            for (let i = 0; i < dataFile.length; i++) {
                const compressed = await compressBase64(dataFile[i], 800, 0.7);
                gambar.push(compressed);
            }
            ...
            $('.top-right').notify({
                message: { text: `Sukses! Barang berhasil ditambahkan!` }
            }).show();
        ...
        function sumTotalItem() {
            ...
            const amount_payment = $('#amount_payment').val();
            const split_amount_payment = $('#split_amount_payment').val();
            const dp = amount_payment + split_amount_payment;
            let total = 0;
            let discount = 0;
        ...
        function createOrder() {
            ...
            const amount_payment = $('#amount_payment').val().replace(/\./g, '');
            const split_amount_payment = $('#split_amount_payment').val().replace(/\./g, '');
            ...
            const split_payment_method = $('#split_payment_method').val();
            const split_payment_merchant = $('#split_payment_merchant').val();
        ...
        $(function() {
            ...
            $('#dp, #discount, #amount_payment, #split_amount_payment').on('input', function(e) {
