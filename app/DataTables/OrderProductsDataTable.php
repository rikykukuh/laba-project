...
class OrderProductsDataTable extends DataTable
{
    ...
    public function dataTable($query): DataTableAbstract
    {
        ...
        // if ($segment) {
            ...
            // });
            $datatables->editColumn('bruto', fn($row) => $row->status == 'CANCEL' ? 0 : $row->bruto);
            $datatables->editColumn('discount', fn($row) => $row->status == 'CANCEL' ? 0 : $row->discount);
            $datatables->editColumn('netto', fn($row) => $row->status == 'CANCEL' ? 0 : $row->netto);
            
            $datatables->addColumn('bruto_formatted', fn($row) =>
                'Rp. ' . number_format($row->status == 'CANCEL' ? 0 : $row->bruto, 0, ',', '.')
            );
            $datatables->addColumn('discount_formatted', fn($row) =>
                'Rp. ' . number_format($row->status == 'CANCEL' ? 0 : $row->discount, 0, ',', '.')
            );
            $datatables->addColumn('netto_formatted', fn($row) =>
                'Rp. ' . number_format($row->status == 'CANCEL' ? 0 : $row->netto, 0, ',', '.')
            );
    ...
    protected function getColumns(): array
    {
        ...
        $columns = [
            ...
            Column::make('number_ticket')->title('No Bon')->addClass( 'text-center' )->width(60),
