...
class OrderProductController extends Controller
{
    ...
    public function create()
    {
        ...
        return view('order-products.create', compact('statuses', 'customers', 'products', 'payment_methods', 'payment_merchants', 'sites', 'cities', 'config'));
    }
    ...
    public function store(Request $request)
    {
        ...
        for ($i = 0; $i < count($items); $i++) {
            ...
            $jumlah_per_qty = $jumlah / $kuantitas;
            if($discount_item > 100) {
                $total_discount_item = (int) $items[$i]['discount_item'];
            } else {
                $total_discount_item = ($jumlah) * $discount_item / 100;
            }

            $amount = ($jumlah - $total_discount_item);
            $vatItem = calculate_included_vat($amount, $config->vat);

            $vatAmount = calculate_included_vat($jumlah, $config->vat);
            $order_item = OrderItem::create([
                'order_id' => $order->id,
                'product_id' => $items[$i]['type'],
                'note' => $items[$i]['keterangan'],
                'transaction_type' => 1,
                'bruto' => $jumlah_per_qty,
    ...
    public function show($id)
    {
        ...
        return view('order-products.show', compact('order', 'customers', 'statuses', 'payment_methods', 'payment_merchants', 'products', 'sites', 'config'));
    }
